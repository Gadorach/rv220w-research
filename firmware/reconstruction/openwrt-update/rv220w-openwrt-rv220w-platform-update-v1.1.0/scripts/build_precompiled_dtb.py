#!/usr/bin/env python3
"""Build the conservative RV220W DTB by pruning the exact OpenWrt 6.12 octeon_3xxx.dtb."""
...
def main():
    ...
    uart=soc.child('serial@1180000000800'); uart.props['status']=sprop('okay')
    ...
    for idx,name in enumerate(('ethernet@0','ethernet@1','ethernet@2')):
        ...
        if idx<2:
            e.props['rx-delay']=be32(0);e.props['tx-delay']=be32(0)
            fl=e.child('fixed-link') or e.add(Node('fixed-link'));fl.props['speed']=be32(1000);fl.props['full-duplex']=b''
        else:e.remove('fixed-link')
    i1=pip.child('interface@1');
    if i1:i1.children=[]
    boot=soc.child('bootbus@1180000000000'); boot.props['status']=sprop('disabled'); boot.remove('led-display@4,0');boot.remove('compact-flash@5,0')
    ...
    rep={'input':str(Path(a.input)),'input_sha256':hashlib.sha256(Path(a.input).read_bytes()).hexdigest(),'output':str(Path(a.output)),'output_sha256':hashlib.sha256(out).hexdigest(),'output_size':len(out),'model':'Cisco RV220W','compatible':['cisco,rv220w','cavium,octeon-3860'],'usb':'disabled','dt_bootbus':'disabled; legacy phys_mapped_flash retained','switch':{'compatible':'brcm,bcm53115','mdio_address':'0x1e','status':'disabled'},'reset':{'gpio':1,'active_low':True,'status':'disabled'},'ethernet':{'interface':0,'ports':[0,1,2],'fixed_1000_full':[0,1]},'validation':'FDT v17 serialized and parsed back successfully'}
