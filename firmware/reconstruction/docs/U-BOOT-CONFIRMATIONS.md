...
## Hardware-validated boot-policy patches

The physical recovery input is GPIO 1 active low:

```text
released: PUSH_BUTTON--> 1
held:     PUSH_BUTTON--> 0 -> Sercomm download mode
```

The post-check branch at `0xffffffffbfc13138` was changed from
`04 41 00 03` (`bgez v0,+3`) to `10 00 00 03` (unconditional branch to the
same success target). This skips only invalid-format HTTP recovery; the earlier
button-driven download path remains functional.

The final late-init tail call at `0xffffffffbfc13168` was changed from
`03 20 00 08` (`jr t9`) to `03 e0 00 08` (`jr ra`). A saved custom `bootcmd`
then persisted across `saveenv` and reset.

Final sector-0 identity:
...
With the button released, the checker may still print invalid-header details,
but U-Boot proceeds to the saved command and boots OpenWrt. With the button
held, Sercomm download recovery remains available.
