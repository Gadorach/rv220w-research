#!/usr/bin/env python3
"""Read or change the RV220W BCM53115 port EAP mode over UART.

The uploaded target helper restricts writes to EAP mode bits 52:51 for
external user ports 0..4 and preserves every other bit.  Changes are RAM-only.
"""
...
from rv220w_uart_collect_b53 import (
    UartError,
    UartShell,
    octal_escape,
    shell_quote,
...
REMOTE_INSTALLED = "/usr/sbin/rv220w-b53-eap-mode"
REMOTE_TEMP = "/tmp/rv220w-b53-eap-mode"
SAFE_BUS = re.compile(r"^[A-Za-z0-9._:-]+$")
ACTIONS = ("show", "show-all", "set-basic", "set-simplified", "restore")
...
def upload_helper(shell: UartShell, helper: pathlib.Path) -> str:
    ...
    if result.status != 0:
        raise UartError("could not clear temporary EAP helper")
...
def main() -> int:
    args = parse_args()
    if not 0 <= args.port <= 8:
        raise UartError("port must be 0..8")
    ...
    if not helper.is_file():
        raise UartError(f"packaged EAP helper not found: {helper}")

    transcript_path = args.transcript
    if transcript_path is None:
        transcript_path = pathlib.Path.cwd() / (
            f"rv220w-b53-eap-{args.action}-p{args.port}-{utc_stamp()}.serial.log"
        )
    ...
    try:
        ...
        command = (
            ...
            f"{args.port} {shell_quote(args.bus)}"
        )
        result = shell.run(command, timeout=args.timeout)
        if result.status != 0:
            raise UartError(
                f"target EAP helper failed with status {result.status}:\n{result.output}"
            )
        print(result.output)
