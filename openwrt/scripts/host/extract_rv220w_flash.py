...
The JFFS2 data partition contains device-specific credentials and is treated as
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-jffs2", action="store_true", help="do not extract the sensitive JFFS2 partition")
...
def main() -> int:
    ...
    data_output = extracted / "data-jffs2-SENSITIVE"
    if not args.no_jffs2:
        ...
        extraction_status["jffs2"] = {"returncode": result.returncode, "path": str(data_output), "sensitive": True}
        if result.returncode != 0:
    ...
    if data_output.exists():
        ...
        # Do not hash or print sensitive file contents.
