#!/usr/bin/env python3
"""Run the RV220W passive B53 topology scanner and retrieve its report over UART.
...
sentinels.  The target-side scanner uses only pseudo-PHY page/address selector writes and switch data reads.
"""
...
REMOTE_INSTALLED: Final[str] = "/usr/sbin/rv220w-b53-snapshot"
REMOTE_TEMP: Final[str] = "/tmp/rv220w-b53-snapshot"
SAFE_LABEL = re.compile(r"^[A-Za-z0-9._-]+$")
SAFE_BUS = re.compile(r"^[A-Za-z0-9._:-]+$")
REMOTE_OUTPUT_RE = re.compile(r"/tmp/rv220w-b53-[A-Za-z0-9._-]+\.txt")
...
def choose_remote_script(shell: UartShell, fallback: pathlib.Path, force_upload: bool) -> str:
    ...
    if result.status != 0:
        raise UartError(f"could not inspect target B53 scanner path:\n{result.output}")
...
def collect(shell: UartShell, remote_script: str, label: str, bus: str) -> tuple[str, str]:
    ...
    if report.status != 0:
        raise UartError(f"could not retrieve target B53 report over UART:\n{report.output}")
    if not report.output.startswith("RV220W passive B53 topology snapshot"):
...
def main() -> int:
    ...
    if not scan_script.is_file():
        raise UartError(f"packaged B53 scanner not found: {scan_script}")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    transcript_path = args.transcript
    if transcript_path is None:
        transcript_path = args.output_dir / f"rv220w-b53-uart-{label}-{utc_stamp()}.serial.log"
