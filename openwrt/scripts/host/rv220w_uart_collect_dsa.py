#!/usr/bin/env python3
"""Run the RV220W DSA validation snapshot tool and retrieve its report over UART.
...
sentinels.  The target-side tool uses only normal Linux networking, sysfs, bridge, ethtool, UCI, and kernel-log reads; it performs no raw MDIO access.
"""
...
REMOTE_INSTALLED: Final[str] = "/usr/sbin/rv220w-dsa-snapshot"
REMOTE_TEMP: Final[str] = "/tmp/rv220w-dsa-snapshot"
SAFE_LABEL = re.compile(r"^[A-Za-z0-9._-]+$")
REMOTE_OUTPUT_RE = re.compile(r"/tmp/rv220w-dsa-[A-Za-z0-9._-]+\.txt")
...
def choose_remote_script(shell: UartShell, fallback: pathlib.Path, force_upload: bool) -> str:
    ...
    if result.status != 0:
        raise UartError(f"could not inspect target DSA snapshot tool path:\n{result.output}")
...
def collect(shell: UartShell, remote_script: str, label: str) -> tuple[str, str]:
    run = shell.run(
        f"{remote_script} {shell_quote(label)}",
        timeout=180.0,
    )
    if run.status != 0:
        raise UartError(f"target DSA snapshot tool failed with status {run.status}:\n{run.output}")
    matches = REMOTE_OUTPUT_RE.findall(run.output)
    if not matches:
        raise UartError(f"target did not return a DSA output path:\n{run.output}")
    remote_output = matches[-1]

    report = shell.run(f"cat {shell_quote(remote_output)}", timeout=60.0)
    if report.status != 0:
        raise UartError(f"could not retrieve target DSA report over UART:\n{report.output}")
    valid_headers = (
        "RV220W DSA validation snapshot",
        "RV220W LAN-only DSA validation snapshot",
    )
    if not report.output.startswith(valid_headers):
...
def main() -> int:
    ...
    if not scan_script.is_file():
        raise UartError(f"packaged DSA snapshot tool not found: {scan_script}")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    transcript_path = args.transcript
    if transcript_path is None:
        transcript_path = args.output_dir / f"rv220w-dsa-uart-{label}-{utc_stamp()}.serial.log"
    ...
    try:
        ...
        remote_output, report = collect(shell, remote_script, label)
