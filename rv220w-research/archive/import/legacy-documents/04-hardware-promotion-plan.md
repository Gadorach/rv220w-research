...
- Photograph both PCB sides and both sides of the WLAN card. The main-board topside and WLAN topside are complete; their undersides remain required.
- Create a project evidence manifest and hash every capture.
- Verify normal boot and front-panel behavior before probing.

**Exit criteria:** complete photographic record of both sides of both boards, identity record, normal-function baseline, and no modifications.
...
- JP1 is confirmed as the primary UART: physical pin 1 VCC/VREF (leave NC), pin 2 target TX, pin 3 target RX, and pin 4 GND.
- Record the powered JP1.1 VREF voltage without connecting adapter VCC.
- Continue passive characterization of the separate JP2 signal pair.
- Preserve repeatable 115200-8N1 cold-boot logs and the Ctrl-C interruption window.

**Exit criteria:** **met for JP1 console access**; JP2 characterization remains optional follow-on work.
...
- Use `scripts/host/dump_uboot_flash_uart.py` to dump the complete `0xbdc00000..0xbfc00000` window twice.
- Use the confirmed temporary 460800-baud console; this U-Boot build rejects 921600.
- Require target/host CRC32 agreement for each chunk and full image.
- Hash and byte-compare both acquisitions.
- Separately inspect the MTD-named `bootload-env` region and the actual final-sector U-Boot environment pointer.
- Identify bootloader, environment, kernel, rootfs, config, MAC, WLAN calibration, and unpartitioned vendor regions.
